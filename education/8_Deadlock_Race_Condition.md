...
Для закрепления материала выполните практическое задание в проекте `practice/practice-8`.

**Задача:** Реализуйте классы и методы так, чтобы все unit-тесты в `DeadlockRaceConditionTest.java` проходили.
...
5. Используйте `ReentrantLock` с тайм-аутом для предотвращения дедлока

**Инструкция:**
1. Перейдите в директорию `practice/practice-8`
2. Запустите тесты: `mvn test`
3. Реализуйте недостающие классы и методы, чтобы все тесты проходили
4. Не изменяйте сами тесты!

**Подсказки:**
- Используйте `AtomicInteger` или `synchronized` для исправления состояния гонки
- Всегда захватывайте ресурсы в одинаковом порядке для предотвращения дедлока
- Используйте `tryLock(timeout)` для предотвращения бесконечного ожидания
- Помните: `volatile` обеспечивает видимость, но не атомарность