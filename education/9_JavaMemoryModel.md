...
**Инструкция:**
1. Перейдите в директорию `practice/practice-9`
...
- `synchronized` гарантирует и видимость, и атомарность для блока кода
- `Thread.start()` создает happens-before отношение
- `Thread.join()` создает happens-before отношение
